 ...
 * @最后修改时间: 2022-11-08 18:09:14
 * @项目的路径: \360-manager-H5\mock\index.js
...
module.exports.init = function (app) {
    // 常用页面demo 分页查询
    app.post("/common/demo/queryPageList", function (request, response) {
    ...
    // 获取配置数据
    app.post("/common/demo/config.do", function (request, response) {
        response.json(
            wrapResponse({
                "activities|10-20": [
                    {
    ...
    // 路由页面demo 分页查询(资源位-我的素材)
    app.post("/router/demo/queryPageList", function (request, response) {
        const time = new Date().getTime();
        response.json(
            wrapResponse({
                "total|10-1000": 0,
